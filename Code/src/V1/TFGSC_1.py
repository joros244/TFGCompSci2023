...
class TFGSC_1(SimplexCalculator):
    def __init__(self, M: List[List[int]]):
        """
        A TFGSC_1 is defined as a DAG with a weight matrix.
        """
        self.G = DAG(len(M))
        self.M = M
    ...
    def transitive_closure(self) -> TFGSC_1:
        """
        Computes the transitive closure of the TFGSC_1.

        Returns
        -------
        TFGSC_1
            The transitive closure of the TFGSC_1 defined by the
    ...
    def __complete_matrix(self):
        """
        Completes the weight matrix using the underlying DAG.
        """
        for i in range(len(self.M)):
    ...
    def filtration(self, r: float) -> TFGSC_1:
        """
        Computes a new TFGSC_1 by deleting the entries of the matrix which
        are less than 'r'.
    ...
    def compute(self, t: float, save_path: str):
        """
        Computes the simplicial complex of the TFGSC_1 using t as a
