...
class TFGSC_5(SimplexCalculator):
    def __init__(self, d: Dict[int, Dict[int, float]], t: List[float]):
        """
        TFGSC_5 is defined by a list of threshold values, the dict which
    ...
    def compute(self, t: float, save_path: str):
        """
        Computes the simplicial complex of the TFGSC_5 using t as a
    ...
    def __search_simplex(self, i: Dict[int, float], c: List[int] = [], q: float = 1):
        """
        Performs a DFS over TFGSC_5's DAG to find the maximal simplices from the
        vertex i.
